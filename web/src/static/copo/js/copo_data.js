...
$(document).ready(function () {
    ...
    //show alerts
    $("#collapseAlertTutorial").collapse('show');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            set_wizard_howtos(data.wiz_howtos);
            set_wizard_summary();
        },
        error: function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (!get_apply_check_state(activeStageIndx)) {
                            $('#wizard_form_' + activeStageIndx).html(wizardStagesForms(data.stage.stage));
                            refresh_tool_tips();
            ...
            for (var i = 0; i < data.stages.length; ++i) {
                if (data.stages[i].stage.title) {
                    ...
                    panes.push({
                        ...
                        pane: get_pane_content(wizardStagesForms(data.stages[i].stage), data.stages[i].stage.message, currentIndx)
                    });
            ...
            $('#dataFileWizard').wizard('addSteps', currentIndx, [
                {
                    ...
                    pane: get_pane_content(wizardStagesForms(data.stage.stage), data.stage.stage.message, currentIndx)
                }
            ]);
    ...
    function get_pane_content(stage_content, stage_message, currentIndx) {
        var stageHTML = '<div id="stage-controls-div-' + currentIndx + '">';

        //stage introductory message
        stageHTML += '<div class="text-default alert alert-default" style="line-height: 150%;">';
        stageHTML += stage_message;
        stageHTML += '</div>';

        //stage form controls
        stageHTML += '<div class="panel panel-info" style="margin-top: 5px; font-size: 12px;">';
        stageHTML += '<div class="panel-heading" style="background-image: none;"><strong>Description Metadata</strong></div>';
        stageHTML += '<div class="panel-body">';
        ...
        var spanMessage = $('<span/>', {
            ...
            html: "Apply this description to all the items in the bundle?"
        });
        ...
        var rightColumn = $('<div/>', {
            ...
            html: '<i data-toggle="collapse" href="#collapseAlert" title="Toggle display" class="fa fa-bell pull-right control-message-trigger" style="cursor: pointer; cursor: hand; font-weight: 800;"></i>'
        });
        ...
        stageHTML += $('<div/>').append(panelGroup).html();


        //form
        stageHTML += '<div style="margin-top: 20px;"><form role="form" id="wizard_form_' + currentIndx + '">';
        ...
        //description bundle
        stageHTML += '<div class="panel panel-info" style="margin-top: 25px; font-size: 12px;">';
        stageHTML += '<div class="panel-heading" style="background-image: none;"><strong>Description Bundle</strong></div>';
        stageHTML += '<div class="panel-body">';
        stageHTML += '<table id="description_target_table_' + currentIndx + '" class="display copo-datatable copo-table-header" cellspacing="0" width="100%">';
    ...
    function initiate_wizard() {
        ...
        //hide wizard tutorial...user may choose to display it later

        $("#collapseAlertTutorial").closest(".wizard-message-panel").find("#collapseAlertTutorial").collapse('hide');
    }
    ...
    function add_to_batch(batchTargets, silence) {
        // validate items in batchTargets before addition.
        // one reason is to avoid duplication of items in description bundle.
        // but also, can the items be bundled together in actual sense?
        // what of inheriting metadata from already existing bundle items?

        // one can also silence (i.e = true) add to batch if you are only refreshing the wizard without necessarily
    ...
    function do_record_task(elem) {
        ...
        if (task == "describe" && records.length > 0) {
            ...
            do_component_delete_confirmation("datafile", ids);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (!appliedToAllFlag) {
                                ...
                                contentHtml += wizardStagesForms(data.stage.stage);
                                contentHtml += '</form>';
                                contentHtml += '</div>';
                                contentHtml += '</div>';
    ...
    function set_wizard_howtos(howTos) {
        var topicDiv = $('<div/>');
        ...
        var anchorToggle = $('<a/>', {
            ...
            "data-parent": "#wizard_tutorials_collapsible"
        });

        panelHeading.append(panelTitle.append(anchorToggle));
        ...
        var panelBody = $('<div/>', {
            class: "panel-body",
            style: "border-color: #ccc; line-height: 1.4;"
        });

        panelCollapse.append(panelBody);

        panelInfo.append(panelHeading).append(panelCollapse);
        topicDiv.append(panelInfo);


        howTos.forEach(function (item, indx) {
            var itemDiv = topicDiv.clone();
            var idConstruct = "wizard_tutorial_collapse" + indx;
            itemDiv.find(".panel-title").find("a").attr("href", "#" + idConstruct);
            itemDiv.find(".panel-title").find("a").html(item.title);
            itemDiv.find(".panel-collapse").attr("id", idConstruct);
            itemDiv.find(".panel-collapse").find(".panel-body").html(item.content);

            $("#wizard_tutorials_collapsible").append(itemDiv.html());
        });
    }

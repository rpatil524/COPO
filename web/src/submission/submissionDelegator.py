...
def delegate_submission(request):
    ...
    if repo == 'figshare':
        ...
        if figshareSubmission.FigshareSubmit().isValidCredentials(user_id=request.user.id):

            token = fda.Figshare().get_token_for_user(request.user.id)['token']
            figshareSubmission.FigshareSubmit().submit(
                sub_id=sub_id,
